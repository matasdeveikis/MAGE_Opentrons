...
tiprack_20 = [
        ...
        for s in [10, 3]]   # its better to use 95 oligos instead of 96 as the you don't need to have a tiprack in module 10 that way
                            # as 
...
# Variables
plasmid_conc = input("Input the CRISPR plasmid concentration in ug/mL: ")
oligos = input("Input number of mutagenic oligos to be used: ")
growth_temp = input("Input incubation temperature for the chosen bacteria: ")
...
p20.transfer(50/float(plasmid_conc), CRISPR_plasmid, storage_oligos.columns()[0:12], touch_tip = True, new_tip = 'always', mix_after = (3, 15))
...
if electroporation == False:
    ...
    protocol.delay(seconds = 15 * 60 - (math.ceil(float(oligos)/8)*delay_per_column)) 
    ...
    protocol.delay(seconds = 90 - (math.ceil(float(oligos)/8)*delay_per_column))  
    ...
    temp_hot.set_temperature(float(growth_temp))
    protocol.delay(seconds = 5 * 60 - (math.ceil(float(oligos)/8)*delay_per_column)) 
    ...
    protocol.delay(seconds = 60 * 60 - (math.ceil(float(oligos)/8)*delay_per_column))
    temp_hot.deactivate()
